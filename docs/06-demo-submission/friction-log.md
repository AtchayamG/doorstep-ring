...
* **Workaround**: Took the frame from WHEP instead. A `werift` capture-client offer (VP8 plus one H264 profile, `42e01f`) returned **HTTP 500**. The same session built by headless Chrome, whose offer lists every H264 profile, returned **201**, negotiated H264 `64001f`, and delivered a 1280x720 frame (`ops/capture-ring-browser.mjs`, 2026-09-23). A 500 for an offer the server cannot match looks like a server fault. A 4xx naming the unsupported codec would have saved a diagnosis round.
...
* **Severity**: Medium.
* **Workaround**: Built a multimodal pipeline that currently uses clearly labelled local fixtures with AWS Bedrock Nova Pro (`amazon.nova-pro-v1:0`) and prompt guardrails (1 sentence, present tense, zero identity/motive speculation). One live Ring Playground frame has now entered the pipeline (2026-09-23): after a prompt fix it is described 6 of 6 times as "A brown package is on the snowy steps."
